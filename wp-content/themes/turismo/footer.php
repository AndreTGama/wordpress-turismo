<?php
$argsContact = array(
	'post_type'      => 'contact_whatsapp',
	'posts_per_page' => 1,
	'orderby'        => 'date',
	'order'          => 'ASC',
);

$contact = get_posts($argsContact);
$cellphone = get_field('cellphone', $contact[0]->ID);
$cellphoneFormat = "+55 (" . substr($cellphone, 2, 2) . ") " . substr($cellphone, 4, 5) . "-" . substr($cellphone, 9);
$message = get_field('message', $contact[0]->ID);
?>

<footer class="text-white py-6">
	<a href="https://wa.me/<?= $cellphone ?>?text=<?= $message ?>"
		class="whatsapp-float"
		target="_blank">
		<img src="https://upload.wikimedia.org/wikipedia/commons/6/6b/WhatsApp.svg"
			alt="WhatsApp" />
	</a>

	<div class="container mx-auto flex flex-col md:flex-row justify-between items-center md:items-start">
		<div class="flex flex-col mb-4 md:mb-0 text-center md:text-left">
			<h3 class="text-lg font-semibold mb-2">Redes Sociais</h3>
			<div class="flex space-x-4 justify-center md:justify-start">
				<a target="_blank" rel="noopener" href="https://www.facebook.com/smecrz/?locale=pt_BR" class="hover:shadow-lg hover:bg-gray-700 p-2 rounded transition duration-300">
					<svg fill="white" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 24 24">
						<path d="M12,2C6.477,2,2,6.477,2,12c0,5.013,3.693,9.153,8.505,9.876V14.65H8.031v-2.629h2.474v-1.749 c0-2.896,1.411-4.167,3.818-4.167c1.153,0,1.762,0.085,2.051,0.124v2.294h-1.642c-1.022,0-1.379,0.969-1.379,2.061v1.437h2.995 l-0.406,2.629h-2.588v7.247C18.235,21.236,22,17.062,22,12C22,6.477,17.523,2,12,2z"></path>
					</svg>
				</a>
				<a target="_blank" rel="noopener" href="https://instagram.com/secretaria.ed?igshid=YmMyMTA2M2Y=" class="hover:shadow-lg hover:bg-gray-700 p-2 rounded transition duration-300">
					<svg fill="white" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 30 30">
						<path d="M 9.9980469 3 C 6.1390469 3 3 6.1419531 3 10.001953 L 3 20.001953 C 3 23.860953 6.1419531 27 10.001953 27 L 20.001953 27 C 23.860953 27 27 23.858047 27 19.998047 L 27 9.9980469 C 27 6.1390469 23.858047 3 19.998047 3 L 9.9980469 3 z M 22 7 C 22.552 7 23 7.448 23 8 C 23 8.552 22.552 9 22 9 C 21.448 9 21 8.552 21 8 C 21 7.448 21.448 7 22 7 z M 15 9 C 18.309 9 21 11.691 21 15 C 21 18.309 18.309 21 15 21 C 11.691 21 9 18.309 9 15 C 9 11.691 11.691 9 15 9 z M 15 11 A 4 4 0 0 0 11 15 A 4 4 0 0 0 15 19 A 4 4 0 0 0 19 15 A 4 4 0 0 0 15 11 z"></path>
					</svg>
				</a>
				<a target="_blank" rel="noopener" href="https://www.youtube.com/@secretariamunicipaldeeduca87" class="hover:shadow-lg hover:bg-gray-700 p-2 rounded transition duration-300">
					<svg fill="white" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 50 50">
						<path d="M 44.898438 14.5 C 44.5 12.300781 42.601563 10.699219 40.398438 10.199219 C 37.101563 9.5 31 9 24.398438 9 C 17.800781 9 11.601563 9.5 8.300781 10.199219 C 6.101563 10.699219 4.199219 12.199219 3.800781 14.5 C 3.398438 17 3 20.5 3 25 C 3 29.5 3.398438 33 3.898438 35.5 C 4.300781 37.699219 6.199219 39.300781 8.398438 39.800781 C 11.898438 40.5 17.898438 41 24.5 41 C 31.101563 41 37.101563 40.5 40.601563 39.800781 C 42.800781 39.300781 44.699219 37.800781 45.101563 35.5 C 45.5 33 46 29.398438 46.101563 25 C 45.898438 20.5 45.398438 17 44.898438 14.5 Z M 19 32 L 19 18 L 31.199219 25 Z"></path>
					</svg>
				</a>
			</div>
		</div>

		<div class="flex flex-col mb-4 md:mb-0 text-center md:text-left">
			<h3 class="text-lg font-semibold mb-2">Mapa do Site</h3>
			<ul class="flex flex-col items-center md:items-start">
				<?php
				$menu_items = wp_get_nav_menu_items('primary-header');

				if ($menu_items) {
					foreach ($menu_items as $item) {
						echo sprintf(
							'<li><a href="%s" class="hover:text-gray-400">%s</a></li>',
							esc_url($item->url),
							esc_html($item->title)
						);
					}
				}
				?>
			</ul>
		</div>

		<div class="flex flex-col mb-4 md:mb-0 text-center md:text-left">
			<h3 class="text-lg font-semibold mb-2">Contato</h3>
			<p>Email: contato@exemplo.com</p>
			<p>Telefone: <?= $cellphoneFormat ?> </p>
		</div>
	</div>
</footer>
<!-- Js -->
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/menu.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/count.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/js/carrousel.js"></script>
<?php wp_footer(); ?>

</body>

</html>